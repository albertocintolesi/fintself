...
def _make_debit_cell(
	date_str: str, type_str: str, description: str, amount_str: str
) -> MagicMock:
	"""Builds a mock for a debit card amount cell; its parent chain is the row."""
	date_loc = MagicMock()
	...
	row = MagicMock()
	row.locator.side_effect = lambda sel: (
		date_loc
		if "nth-child(1)" in sel
		else type_loc if "nth-child(2)" in sel else desc_loc if "nth-child(3)" in sel else MagicMock()
	)

	cell = MagicMock()
	cell.inner_text.return_value = amount_str
	cell.locator.return_value = row  # any call (xpath=../..) → row
	return cell


def _setup_debit_page(scraper: BiceScraper, cells: list) -> None:
	...
	cells_locator.all.return_value = cells

	next_button = MagicMock()
	next_button.is_visible.return_value = False

	def _locator(sel):
		if sel == amount_cell_selector:
...
class TestExtractDebitCardMovements:
	def test_cargo_is_negative(self, scraper):
		cell = _make_debit_cell("15 mar 2025", "Cargo", "Supermercado Ejemplo", "50.000")
	...
	def test_abono_is_positive(self, scraper):
		cell = _make_debit_cell("10 ene 2025", "Abono", "Transferencia recibida", "100.000")
		_setup_debit_page(scraper, [cell])
		movements = scraper._extract_debit_card_movements()
		assert len(movements) == 1
	...
	def test_empty_page_returns_empty(self, scraper):
		scraper._wait_for_selector = MagicMock(side_effect=DataExtractionError("timeout"))
		movements = scraper._extract_debit_card_movements()
		assert movements == []

	def test_zero_amount_is_skipped(self, scraper):
		cell = _make_debit_cell("15 mar 2025", "Cargo", "Anulado", "0")
		_setup_debit_page(scraper, [cell])
		movements = scraper._extract_debit_card_movements()
		assert movements == []
...
def _setup_unbilled_national_page(scraper: BiceScraper, rows: list) -> None:
	...
	page.locator.side_effect = lambda sel: rows_locator if sel == row_selector else MagicMock()
...
def _make_accordion(period_label: str, rows: list, is_expanded: bool = False) -> MagicMock:
